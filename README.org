...
Isogeny provides two means of rendering the values of environment variables in templates, a custom ~env~ tag used in the template like ~{% env "PATH" %}~, or in the config ~{:path #=(java.lang.System/getenv "PATH")}~ (requires ~--eval~).

*** Executing code
When using an EDN context and the ~--eval~ flag, Clojure's ~#=()~ reader macro can be used to execute code at read time:
...
Without ~--eval~, contexts are pure data and ~#=()~ will throw an exception. Classes must be fully qualified (e.g. ~java.lang.System~, not ~System~).
